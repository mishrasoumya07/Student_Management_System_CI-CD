...
export default function AllStudents() {
  ...
  // 2. Logic for Global Search Modal
  const globalSearchResults = students.filter((student) => {
  ...
  const handleUpdate = async (e) => {
    ...
    if (!editData.age || Number(editData.age) <= 0 || Number(editData.age) > 120) {
    ...
    try {
      ...
      setActionError(
        err.response?.data?.detail || "Unable to update student. Please try again."
      );
    } finally {
  ...
  const handleDelete = async () => {
    try {
      ...
      setActionError(
        err.response?.data?.detail || "Unable to delete student. Please try again."
      );
    } finally {
      setActionLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-[#f6f7fb]">
      ...
      >
        ...
        <div className="px-5 pt-7">
          ...
          <nav className="space-y-1">
            <SidebarItem icon={<LayoutDashboard size={19} />} label="Overview" to="/" />
            <SidebarItem active icon={<Users size={19} />} label="Students" to="/students" />
            <SidebarItem icon={<UserPlus size={19} />} label="Add Student" to="/create-student" />
          </nav>
        </div>
        ...
        <div className="px-5 pt-8">
          ...
          <nav className="space-y-1">
            ...
            <SidebarItem icon={<HelpCircle size={19} />} label="Support" />
          </nav>
        </div>
      ...
      <main className="lg:pl-[280px]">
        ...
        <div className="mx-auto max-w-[1450px] p-4 md:p-8">
          ...
          <section className="mb-8 flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-xl leading-relaxed text-slate-500">
                Manage, update and maintain student records from your centralized academic database.
              </p>
            </div>
          ...
          <section className="mb-6 grid gap-4 sm:grid-cols-3">
            <MiniStat icon={<Users size={19} />} label="Total Students" value={students.length} />
            <MiniStat icon={<Database size={19} />} label="Visible Records" value={filteredStudents.length} />
            <MiniStat icon={<CheckCircle2 size={19} />} label="Database Status" value="Healthy" success />
          </section>
          ...
          <section className="overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-sm">
            ...
            <div className="flex flex-col gap-4 border-b border-slate-100 p-5 md:flex-row md:items-center md:justify-between md:p-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">Search, update or remove student records.</p>
              </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <div className="relative">
                  ...
                  <ArrowUpDown size={16} className="pointer-events-none absolute left-3 top-3 text-slate-400" />
                </div>
            ...
            {loading ? (
              <div className="flex min-h-[400px] flex-col items-center justify-center gap-4">
                ...
                <div className="text-center">
                  <p className="font-semibold text-slate-700">Loading students</p>
                  <p className="mt-1 text-sm text-slate-400">Retrieving student records...</p>
                </div>
              </div>
                ...
                <table className="w-full min-w-[850px]">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/70">
                      <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-slate-400">Student</th>
                      <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-slate-400">Email</th>
                      <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-slate-400">Age</th>
                        ...
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="mt-1 text-xs text-slate-400">Student ID #{student.id}</p>
                            </div>
                          </div>
                        </td>
                        {/* EMAIL */}
            ...
            {!loading && filteredStudents.length > 0 && (
              <div className="flex flex-col gap-3 border-t border-slate-100 bg-slate-50/50 px-6 py-4 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between">
                <p>Showing <span className="font-bold text-slate-700">{filteredStudents.length}</span> of <span className="font-bold text-slate-700">{students.length}</span> students</p>
                <div className="flex items-center gap-2 text-xs text-emerald-600">
                  <span className="h-2 w-2 rounded-full bg-emerald-500" /> Database connected
                </div>
              </div>
            )}
          </section>
          ...
          <footer className="flex flex-col gap-3 py-8 text-xs text-slate-400 md:flex-row md:items-center md:justify-between">
            <p>EduCore Student Management Platform</p>
            <p>Secure centralized student database</p>
          </footer>
        </div>
      </main>
      ...
      {/* ================= SEARCH MODAL (New) ================= */}
      {searchOpen && (
        <div className="fixed inset-0 z-[120] flex items-start justify-center bg-slate-950/40 p-4 pt-24 backdrop-blur-sm">
          <div className="w-full max-w-xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
            ...
            <div className="p-5 max-h-[350px] overflow-y-auto">
              {searchQuery === "" ? (
                <p className="text-xs font-semibold text-slate-400">Search across your student database</p>
              ) : globalSearchResults.length > 0 ? (
                <div className="flex flex-col gap-2">
                  {globalSearchResults.map((student) => (
                    <div key={student.id} className="flex items-center justify-between rounded-xl border border-slate-100 p-3 transition hover:bg-slate-50">
                      <div>
                        <p className="text-sm font-bold text-slate-800">{student.name}</p>
                        <p className="text-xs text-slate-400">{student.email}</p>
                      </div>
                      <div className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md">
                        Age: {student.age}
                      </div>
                    </div>
                  ))}
                ...
                <p className="text-xs font-semibold text-slate-400">No students found matching "{searchQuery}"</p>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="w-full max-w-lg overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-900">Update Student</h2>
                  <p className="mt-1 text-sm text-slate-500">Modify student information and save changes.</p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleUpdate} className="p-6">
              ...
              <div className="space-y-5">
                <EditField label="Student Name" icon={<User size={18} />} type="text" name="name" value={editData.name} onChange={handleEditChange} placeholder="Student name" />
                <EditField label="Email Address" icon={<Mail size={18} />} type="email" name="email" value={editData.email} onChange={handleEditChange} placeholder="student@example.com" />
                <EditField label="Age" icon={<Calendar size={18} />} type="number" name="age" value={editData.age} onChange={handleEditChange} placeholder="Student age" />
              </div>
              <div className="mt-8 flex justify-end gap-3 border-t border-slate-100 pt-6">
                <button type="button" onClick={closeEditModal} disabled={actionLoading} className="rounded-xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 disabled:opacity-50">
                  Cancel
                </button>
                <button type="submit" disabled={actionLoading} className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-200 transition hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-70">
                  {actionLoading ? <><Loader2 size={18} className="animate-spin" /> Saving...</> : <><Save size={17} /> Save Changes</>}
                </button>
              </div>
            </form>
          </div>
      ...
      {deleteModal && selectedStudent && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-7 shadow-2xl">
            ...
            <h2 className="mt-6 text-xl font-bold text-slate-900">Delete Student?</h2>
            <p className="mt-3 leading-relaxed text-slate-500">
              You are about to permanently delete <span className="font-bold text-slate-800">{selectedStudent.name}</span> from your student database.
            </p>
            <div className="mt-5 rounded-xl border border-rose-100 bg-rose-50 p-4">
              ...
              <p className="mt-1 text-xs leading-relaxed text-rose-600">The student record and its associated information will be permanently removed.</p>
            </div>
            {actionError && (
            ...
            <div className="mt-7 flex gap-3">
              <button onClick={closeDeleteModal} disabled={actionLoading} className="flex-1 rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 disabled:opacity-50">
                Cancel
              </button>
              <button onClick={handleDelete} disabled={actionLoading} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-rose-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-rose-200 transition hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-70">
                {actionLoading ? <><Loader2 size={18} className="animate-spin" /> Deleting...</> : <><Trash2 size={17} /> Delete</>}
              </button>
            </div>
          </div>
        </div>
...
function SidebarItem({ icon, label, active = false, to }) {
  const className = `
    ...
    ${active ? "bg-gradient-to-r from-indigo-50 to-violet-50 text-indigo-700" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}
  `;
  if (to) return <Link to={to} className={className}>{icon}<span>{label}</span></Link>;
  return <button className={className}>{icon}<span>{label}</span></button>;
}

/* ================= MINI STAT ================= */
function MiniStat({ icon, label, value, success = false }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        {success && <span className="h-2.5 w-2.5 rounded-full bg-emerald-500" />}
      </div>
...
/* ================= EDIT FIELD ================= */
function EditField({ label, icon, type, name, value, onChange, placeholder }) {
  return (
    <div>
      ...
      <div className="group flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 transition focus-within:border-indigo-500 focus-within:bg-white focus-within:ring-4 focus-within:ring-indigo-500/10">
        <div className="text-slate-400 transition group-focus-within:text-indigo-600">{icon}</div>
        <input type={type} name={name} value={value} onChange={onChange} placeholder={placeholder} min={type === "number" ? "1" : undefined} className="w-full bg-transparent text-sm font-medium text-slate-800 outline-none placeholder:text-slate-400" />
      </div>
    </div>
  );
}